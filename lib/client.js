...
//     /dsh-ssh-remote/ls, choose or type a remote path → /dsh-remote/mirror builds a
...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function RemoteWorkspacePage() {
      ...
      const refresh = () => api('GET', '/dsh-ssh-remote/machines').then((r) => {
      ...
      const checkUpdate = (quiet) => {
        ...
        api('GET', '/dsh-ssh-remote/update-check')
          .then((r) => {
      ...
      const applyUpdateNow = () => {
        ...
        if (!window.confirm('将更新 dsh-ssh-remote 到 v' + upd.latest + '，替换本机插件文件。更新后需重启 Harness 生效。继续？')) return
        setUpdBusy(true); setUpdMsg('')
        api('POST', '/dsh-ssh-remote/update-apply', { version: upd.latest })
          .then((r) => {
      ...
      const saveUpdateMode = (mode) => {
        ...
        api('POST', '/dsh-ssh-remote/update-mode', { mode }).catch((e) => setUpdMsg('模式保存失败: ' + String((e && e.message) || e)))
      }
      React.useEffect(() => { checkUpdate(true) }, [])

      const refreshForwards = () => api('GET', '/dsh-ssh-remote/forwards').then((r) => setForwards(r.forwards || [])).catch(() => {})
      React.useEffect(() => { refreshForwards(); api('GET', '/dsh-ssh-remote/audit?limit=30').then((r) => setAuditLines(r.lines || [])).catch(() => {}) }, [])
      ...
      const save = (action) => {
        ...
        if (action === 'delete') {
          api('POST', '/dsh-ssh-remote/machines', { action: 'delete', id: form.id }).then(refresh).then(() => { clearForm(); setMsg('已删除') }).catch((e) => setErr(String((e && e.message) || e))).finally(() => setBusy(false))
          return
        }
      ...
      const importSsh = () => {
        api('GET', '/dsh-ssh-remote/ssh-config').then((r) => setSshEntries(r && r.present ? (r.entries || []) : []))
      ...
      const setWorkspaceNow = (p) => {
        api('POST', '/dsh-ssh-remote/workspace', { path: p })
      ...
      const fwdAction = (action, id) => {
        api('POST', '/dsh-ssh-remote/forwards', { action, id })
          .then((r) => { setForwards(r.forwards || []); setErr(''); setMsg('') })
          .catch((e) => setErr(String((e && e.message) || e)))
      }
      const fwdDefine = () => {
        if (!fwdForm.listenPort) return
        api('POST', '/dsh-ssh-remote/forwards', { action: 'define', listenPort: Number(fwdForm.listenPort), targetHost: fwdForm.targetHost || '127.0.0.1', targetPort: Number(fwdForm.targetPort) || Number(fwdForm.listenPort), direction: fwdForm.direction, autoStart: fwdForm.autoStart })
      ...
      return React.createElement('div', { style: { padding: 16, display: 'flex', flexDirection: 'column', gap: 12, maxWidth: 860 } },
        ...
        // 快捷切换工作区（该机器的最近工作区）
        workspaces.length ? React.createElement('div', { style: box },
        ...
        React.createElement('div', { style: { display: 'flex', gap: 12, alignItems: 'center', flexWrap: 'wrap', fontSize: 11, opacity: 0.65, borderTop: '1px solid ' + T.border, paddingTop: 10, marginTop: 4 } },
          ...
          React.createElement('a', { href: 'https://github.com/flymysql/dsh-ssh-remote', target: '_blank', rel: 'noopener noreferrer', style: { color: 'inherit' } }, '⭐ 去 GitHub 点个 Star'),
          React.createElement('span', {}, '·'),
          React.createElement('a', { href: 'https://github.com/flymysql/dsh-ssh-remote/issues/new', target: '_blank', rel: 'noopener noreferrer', style: { color: 'inherit' } }, '💬 反馈建议 / 提 issue'),
          React.createElement('span', { style: { marginLeft: 'auto', opacity: 0.5 } }, 'dsh-ssh-remote v' + (upd && upd.current ? upd.current : '?.?.?')),
        ),
      )
    }
    ...
    function DirPicker(props) {
      ...
      const switchTab = (t) => {
        setTab(t); setErr('')
        if (t === 'remote') {
          // 初始路径 = easyssh 的 remoteRoot（与右侧文件树一致），否则回退 '/'。
          // 这样左侧「选择工作目录」从右侧显示的远程根目录开始，路径不再是 '/'。
          fetch('/api/dsh-easyssh/state')
            .then((r) => r.json())
            .then((d) => {
              const st = d && d.state
              const root = st && typeof st.remoteRoot === 'string' && st.remoteRoot !== '' ? st.remoteRoot : '/'
              if (machineId) loadLevels(machineId, root === '/' ? '' : root, 0)
              setPath(root)
...
// appending that directory as the new deepest level (path is pieced together).
      ...
      // createDir(): create a new folder under the current remote path and
      // dive into it (so the user can immediately select it as a workspace).
      const createDir = async () => {
        const name = String(newDir || '').trim()
        if (!name) { setErr('请输入文件夹名称'); return }
        ...
        setLoading(true); setErr('')
        try {
          await api('POST', '/dsh-ssh-remote/mkdir', { machineId, path: target })
          setNewDir('')
          loadLevels(machineId, target, (levels ? levels.length : 0))
        } catch (e) { setErr(String((e && e.message) || e)) }
        finally { setLoading(false) }
      }
      ...
      function renderDirPopup() {
        ...
        return React.createElement('div', {
          ...
          React.createElement('div', { style: { background: v('--dsw-alias-bg-overlay', '#1e1e1e'), border: '1px solid ' + T.borderStrong, borderRadius: 10, boxShadow: '0 10px 40px rgba(0,0,0,0.5)', width: 'min(560px, 94vw)', minWidth: 320, display: 'flex', flexDirection: 'column', maxHeight: 'min(440px, 82vh)', overflow: 'hidden' }, onClick: (e) => e.stopPropagation() },
            ...
            // 新建文件夹：在当前远程路径下创建一个目录并进入其中
            React.createElement('div', { style: { display: 'flex', gap: 6, alignItems: 'center', padding: '6px 12px', borderBottom: '1px solid ' + T.border } },
              React.createElement('span', { style: { fontSize: 11, opacity: 0.75, whiteSpace: 'nowrap' } }, '新建文件夹'),
              React.createElement('input', { value: newDir, onChange: (e) => setNewDir(e.target.value), onKeyDown: (e) => { if (e.key === 'Enter') void createDir() }, placeholder: '名称', spellCheck: false, style: { ...inputS, flex: 1, fontSize: 12 } }),
              React.createElement('button', { style: { ...buttonS, padding: '3px 10px', whiteSpace: 'nowrap' }, onClick: () => void createDir(), disabled: loading || !String(newDir || '').trim() }, '创建'),
            ),
            ...
            err ? React.createElement('div', { style: { color: T.danger, fontSize: 12, padding: '4px 12px 8px', borderTop: '1px solid ' + T.border } }, err) : null,
          ),
        )
      }
    ...
    function apply(ctx) {
      ...
      try { WORKSPACES = (ctx && ctx.get && ctx.get('workspaces')) || null } catch { WORKSPACES = null }
