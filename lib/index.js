...
const routes = [
    ...
    {
      kind: 'exact',
      path: '/dsh-ssh-remote/mkdir',
      handler: async (req, res) => {
        if (req.method !== 'POST') return sendJson(res, 405, { error: 'method not allowed' })
        try {
          const payload = JSON.parse((await readBody(req)) || '{}')
          const machine = resolveMachine(payload.machineId) || currentMachine()
          if (!machine) return sendJson(res, 400, { error: 'no machine selected' })
          const p = normalizeRemotePath(String(payload.path || ''))
          if (!p || p === '/') return sendJson(res, 400, { error: 'a directory path is required' })
          const s = await poolFor(machine).sftp()
          await s.mkdir(p)
