...
//   • /dsh-remote/status 返回全部机器及其连接状态
//   • 新增 rw_switch / rw_conn / rw_disc（切换/连接/断开指定机器）
//
// 底层 helpers（shq / normalizeRemotePath / mirror / syncTree / pushTree /
// machines registry / SshPool）与原版一致，未改动。
  ...
  const tools = [
    ...
    defineTool({
      ...
      async execute(args) {
        ...
        const raw = await run(machine, `sed -n '${from},${to}p' -- ${shq(p)}`, { timeoutMs: config.commandTimeoutMs })
    ...
    },
    {
      ...
      handler: async (req, res) => {
        ...
        try {
          ...
          const r = await poolFor(machine).exec(`if [ -d ${shq(p)} ]; then echo DIR; else echo NOTDIR; fi`)
