...
var SshEngine = class {
	...
	async withClient(alias, fn, attempts = 3) {
		...
		for (let attempt = 1; attempt <= attempts; attempt += 1) {
			...
			try {
				...
				lastError = error;
				if (record.broken || record.inFlight === 1) this.disposeRecord(alias, record);
			} finally {
				record.inFlight -= 1;
			}
		}
	...
	*/
	...
	async doAcquire(alias) {
		const entry = this.store.find(alias);
		if (entry === void 0) throw new Error(`alias '${alias}' not found 鈥?add it first`);
	...
	*/
	...
	async download(alias, remotePath, localPath, onProgress) {
		return this.withClient(alias, async (client) => {
			...
			})).isDirectory()) throw new Error(`'${remotePath}' is a directory 鈥?directory download is not supported yet (download individual files)`);
			const local = resolve(localPath);
		...
		return this.withClient(alias, async (client) => {
			...
			return this.withTimeout((async () => {
				const list = await new Promise((resolve, reject) => {
					sftp.readdir(path, (error, items) => error !== void 0 ? reject(error) : resolve(items));
				});
				return this.classifyEntries(sftp, path, list);
			})(), SFTP_OP_TIMEOUT_MS, `remote ls timed out after ${SFTP_OP_TIMEOUT_MS}ms: ${path}`);
		});
	}
	/**
	...
	*/
	async classifyEntries(sftp, dirPath, list) {
		const resolved = new Array(list.length).fill(null);
		const linkIndexes = list.map((item, index) => item.attrs.isSymbolicLink() ? index : -1).filter((index) => index >= 0);
			...
			await Promise.all(batch.map(async (index) => {
				try {
					const stats = await new Promise((res, rej) => {
						sftp.stat(`${base}/${list[index].filename}`, (statError, stats) => statError !== void 0 ? rej(statError) : res(stats));
					});
					resolved[index] = stats.isDirectory() ? "dir" : stats.isFile() ? "file" : "other";
				} catch {
					resolved[index] = "other";
				}
			}));
		}
	...
	*/
	async readStream(alias, remotePath) {
		return this.withClient(alias, async (client) => {
			return (await this.sftpFor(client)).createReadStream(remotePath);
		});
	}
	/**
	...
	*/
	...
	async startTunnel(alias, options) {
		...
		if (this.store.find(alias) === void 0) throw new Error(`alias '${alias}' not found 鈥?add it first`);
